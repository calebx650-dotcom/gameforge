...
describe("GameForge end-to-end smoke test", () => {
  ...
  it("splices real frames from a reference video when ffmpeg is available, and degrades gracefully with a clear log entry when it isn't (P3 video reference input)", async () => {
    ...
    const videoProjectRoot = await mkdtemp(join(tmpdir(), "gf-e2e-refvideo-"));
    const openRes = await request(httpBase).post("/api/projects").send({ path: videoProjectRoot });
    const projectId = openRes.body.id;

    let videoBase64: string;
    if (ffmpegAvailable) {
      // A real, tiny synthetic video generated by ffmpeg's own lavfi testsrc
      // (no external file dependency needed) so the real extraction path
      // is exercised end to end wherever ffmpeg genuinely exists.
      const tmpVideoPath = join(videoProjectRoot, "ref.mp4");
      await execFileAsync("ffmpeg", ["-y", "-f", "lavfi", "-i", "testsrc=duration=2:size=64x64:rate=2", tmpVideoPath]);
      videoBase64 = (await readFile(tmpVideoPath)).toString("base64");
    } else {
      // Not a real video -- exercises the graceful-degradation path, which
      // is the behavior that matters most in this project's own CI/sandbox
      // environment, where ffmpeg genuinely isn't installed (see
      // packages/vision/src/ffmpeg.test.ts for the same reasoning).
      videoBase64 = Buffer.from("not a real video").toString("base64");
    }

    const result = await new Promise<{ finalText: string; logSummaries: string[]; sawFramesInLocalMessages: boolean }>((resolve, reject) => {
      const ws = new WebSocket(`ws://localhost:${gfPort}/ws/chat`);
      const timeout = setTimeout(() => reject(new Error("reference-video e2e timed out")), 15_000);
      ...
      ws.on("open", () => {
        ws.send(
          JSON.stringify({
            ...
            mode: "ask",
            providerSettings: { provider: "ollama", model: "llama3.1:8b", baseUrl: `http://127.0.0.1:${fakeOllamaPort}` },
            message: "What's happening in this reference video?",
            referenceVideo: { data: videoBase64, mimeType: "video/mp4" },
          }),
        );
      });
      ...
      ws.on("message", (raw) => {
        const msg = JSON.parse(raw.toString());
        if (msg.type === "log") logSummaries.push(msg.entry.summary);
        else if (msg.type === "result") {
          clearTimeout(timeout);
          const finalMessage = msg.messages[msg.messages.length - 1];
          const firstUserMessage = msg.messages[1];
          const sawFramesInLocalMessages =
            Array.isArray(firstUserMessage?.content) && firstUserMessage.content.some((part: { type: string }) => part.type === "image");
          ws.close();
          resolve({ finalText: finalMessage.content, logSummaries, sawFramesInLocalMessages });
        } else if (msg.type === "error") {
          clearTimeout(timeout);
          reject(new Error(msg.message));
        }
    ...
    if (ffmpegAvailable) {
      // Proves real frames extracted by real ffmpeg made it all the way into
      // the message GameForge actually sent, not just that the request didn't crash.
      expect(result.sawFramesInLocalMessages).toBe(true);
    } else {
      expect(result.logSummaries.some((s) => /Reference video frame extraction skipped/.test(s))).toBe(true);
      expect(result.sawFramesInLocalMessages).toBe(false);
    }
    // Either way, the chat request itself still completes -- a video whose
    // frames couldn't be extracted must not take the whole message down with it.
